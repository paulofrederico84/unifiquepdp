...
function DraggableCamera({ cam }) {
  ...
  return (
    <div
      ref={drag}
      className={`flex items-center gap-3 p-3 rounded-lg border bg-white shadow-sm cursor-move hover:shadow-md transition-all ${isDragging ? 'opacity-50' : ''}`}
    >
      ...
      {tab === 'catalog' && (
        <div className="px-6 pb-6">
          ...
          <div className="mt-6 space-y-3">
            {filtered.map(cam => <DraggableCamera key={cam.id} cam={cam} />)}
          </div>
