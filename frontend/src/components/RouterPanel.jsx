...
function ClickableRouter({ rt, onAdd }) {
    return (
        <div onClick={() => onAdd(rt)} className="flex items-center gap-3 p-3 rounded-lg border bg-white shadow-sm cursor-pointer hover:shadow-md hover:border-[#0077B6] transition-all">
...
export default function RouterPanel({ onClose, onAddEquipment, placedEquipments, onDeleteEquipment, onDuplicateEquipment }) {
            ...
            <div className="fixed top-10 left-24 w-[420px] max-h-[calc(100vh-80px)] overflow-y-auto bg-white rounded-3xl shadow-2xl border border-[#d9e7f2] z-40">
                ...
                <div className="px-6 pb-6">
                    ...
                    <div className="mt-6 space-y-3">
                        {filtered.map(rt => <ClickableRouter key={rt.id} rt={rt} onAdd={onAddEquipment} />)}
                    </div>
